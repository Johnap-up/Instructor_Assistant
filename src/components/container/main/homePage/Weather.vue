...
<template>
  <div style="height: 200px;width: 350px" v-loading="!data.success"
       element-loading-text="正在加载天气信息...">
    <div style="display: flex;justify-content: space-between;margin: 20px 20px"
         v-if="data.success">
      <div style="font-size: 65px;color: grey">
        <i :class="`qi-${data.now.icon}-fill`"></i>
      </div>
      <div style="font-weight: bold;text-align: center">
        <div style="font-size: 40px">{{data.now.temp}}℃</div>
        <div style="font-size: 25px">{{data.now.text}}</div>
      </div>
